...
helm upgrade --install gearoff $PSScriptRoot/helm/gearoff --namespace gearoff --create-namespace `
  ...
  --set "serviceBusImageQueueConnectionString=$(az servicebus queue authorization-rule keys list -g $ResourceGroup --namespace-name $sbNamespaceName --queue-name "image-actions" --name "manage-rule" --query primaryConnectionString -o tsv)" `
